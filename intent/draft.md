...
doctor 必须明确显示这些数据会离开本机。日志不得记录全文、完整摘录、完整提示词、认证信息或密钥。开始正式校准前必须确定并引用由 Screening、复用升级和建议规则等相关配置构成的校准语义基线；Read 模型不属于校准输入，只需在运行相应 Read 前明确配置，且不得自动 fallback。校准后若这些校准相关语义输入发生变化，系统先生成待分级变更；由用户附理由判定为 minor 或 major 后才能继续，不能自动决定是否沿用既有校准。
...
运行配置快照（`RuntimeConfigSnapshot`）是所选版本化配置的不可变编译结果，可供多次运行与任务引用，不含某篇论文的输入，也不规定任务执行顺序。各阶段只取快照中与自身语义相关的配置投影，并结合实际论文输入生成阶段输入指纹；指纹只回答“哪些制品需要重算”，不能自行回答“既有校准门禁是否仍有效”。校准语义基线是快照中能影响 Screening、条件式复用升级、摘录产生与筛选建议的配置投影；正式校准还要冻结批次的用户可见输入。仅改变 Read 模型等无关配置可以生成新快照，但不改变校准基线，不使 Screening 制品失效，也不触发校准变更分级。任何被检测为可能改变校准语义或建议边界的配置差异，都必须形成待用户分级的校准变更记录：未分级时暂停自动精读；minor 经审计链继承门禁并按阶段指纹重算受影响制品；major 关闭门禁，等待基于既有人工标签重算或开始新校准。门禁不能简单比较完整快照 ID，也不能仅靠基线哈希变化自动判定 minor 或 major。
...
复用可行性在摘要层的可判断性通常弱于研究价值。阶段二仍保存 1–5 的初步预测，但必须另行声明该预测能否由当前输入充分判断；默认只有研究价值属于版本化参数 `reuse_escalation_research_values`（V1 为 `{3}`）且复用可行性不可判断时，才进入条件式全文摘录升级。参数可以在以后改为 `{2, 3}` 或其他受控集合，但内容与版本必须进入运行配置快照、相关阶段输入指纹和校准语义基线，变化须走校准变更分级，不能原地改写。
...
每日 20 篇自动 Screening 配额中，在预热结束并确定正式校准语义基线后，若正式校准尚未完成、当前没有未揭晓批次且 `waiting_calibration` 存在待筛选成员，则保留 5 篇给校准。`run-due` 不得用新 Feed 或历史晋级任务占用该预留；无待校准工作时当日释放。配额内部优先级固定为“校准批次成员 > 新 Feed 论文 > 晋级历史候选”。`calibration create-batch` 先按上述机械规则确定成员并预检可用及预留名额；缺少与当前校准语义基线对应阶段输入指纹相符的阶段一、二制品时同步生成并计入配额，名额不足则不创建批次。命令不等待 PDF、人工介入或复用升级：只冻结按当前基线已就绪的升级摘录及制品身份；尚无合法摘录者冻结为 `reuse_escalation_unavailable`，已有摘录但升级模型失败者冻结其 `reuse_unknown` 建议和失败类别，后到结果均不回写。阶段一/二模型耗尽两次仍失败时保留成员，以 `pending(reason=model_failure)` 参与比较，不能换入更容易成功的论文。
...
如果前两项不达标，校准未通过。允许依据揭晓结果调整 Profile、提示词、模型、契约、升级参数或规则，然后复用同一组人工标签重新产生制品、建议和门槛指标；每轮必须保存所依据的运行配置快照、校准语义基线、结果和前一轮关系，并标记 `reused_revealed_labels`。若重算所用的题名、摘要、摘录或其他用户可见输入与原批次冻结输入不同，evaluation 还必须标记 `human_input_drift`，明确人工标签不是在新输入上重新作出的。这种结果仍可作为第一版门禁依据，但它是对已揭晓调优样本的拟合后复算，不得称为新样本独立验证。如果明确决策数、system accepts 或 human accepts 任一最小分母不足，结果是“校准不确定”，在校准语义基线不变时把四刊目标各增加 2 篇并按同一轮转算法扩样，直到分母充分。
...
|runtime_config_snapshots|所选版本化配置编译后的内容及身份：Profile、主题、期刊、模型、提示词、规则、升级参数、解析器及确定性算法版本哈希|不可变并可被多次运行和任务引用；不含单篇论文输入或执行顺序|
...
|calibration_evaluations|样本集合、各刊元数据/预热/既有暴露排除及延后/截断计数、运行配置快照引用、校准语义基线身份、各指标分子分母、人工 pending、升级分组、隐含接受率与容量对照、人工输入兼容性、警告、结果、评估依据、前一轮|每次重算只追加；system/human accepts 任一为 5–9 标 small_sample；复用揭晓标签标 reused_revealed_labels，用户可见输入变化再标 human_input_drift|
|calibration_changes|前后运行配置快照及校准语义基线、校准相关差异摘要、待分级/minor/major、用户理由、时间|只对可能影响校准语义的差异记录；检测后先待分级，只有用户能确定 minor/major|
...
- 用户事件与已发布报告成员；
- 所有被运行配置快照或历史制品引用的 Profile、主题、提示词和导出契约版本。
...
当前运行配置快照的校准语义基线与门禁所依据的 evaluation 基线（或其已分级的 minor 继承链末端）存在校准相关差异时，doctor 和 run-due 必须列出差异并要求用户分级。只改变 Read 模型等无关配置时，即使完整快照 ID 变化，也不得因此关闭门禁、重算 Screening 或创建校准变更。系统不得仅依据文件名、版本号或 diff 大小自动判定 minor/major；用户选择和理由形成审计事件。minor 可沿审计链继承既有门禁，major 才关闭既有门禁，但待分级期间自动精读保持暂停。

提示词、Research Profile 和导出的输出契约使用声明版本 + 内容哈希的不可变文件。修改内容必须创建新版本文件并更新 active selector，不能在同一版本路径原地覆盖；doctor 发现同一声明版本对应不同哈希时拒绝编译运行配置快照。旧版本及其实际 Schema 快照永久保留，使校准重算和历史制品可以脱网回放。
...
paper-radar artifacts gc --execute <gc-plan-id>
~~~

清理计划显式列出 artifact ID、路径、未被引用证据和可重建依据。执行只接受已保存的清理计划 ID，不接受目录或 glob。原 PDF、正式分析、模型成功原始响应、有效来源快照，以及被分析、筛选建议、校准成员或复用升级引用的 Docling 制品永不进入清理计划；`<gc-plan-id>` 不是运行配置快照 ID。
...
- 同一声明版本下修改 Profile、提示词或契约内容时运行配置快照编译失败；新增版本并切换 selector 时保留旧文件和旧哈希；
- 只改变 Read 模型时可得到新运行配置快照，但校准语义基线及 Screening 输入指纹不变；
...
- 用用户手工挑选的 10–20 篇明确正反例和边界例完成预热，比较 2–3 个 Screening/reuse-assessment 组合并确定正式校准语义基线；预热样本不得计入门禁；
...
- 预热期产生的制品只有在论文仍具正式样本资格、该阶段输入指纹与正式批次所需输入相同且未泄漏隐藏结果时才能复用；指纹不符时 create-batch 必须生成新制品，无关 Read 配置变化不阻止同指纹复用；
...
- 80% 或 90% 不达标后，复用同一 32 篇标签重算并保留新旧 evaluation、所依据的运行配置快照、校准语义基线和指标；
- 复用揭晓标签的 evaluation 明确标记 reused_revealed_labels，任何输出都不称其为独立验证；
- 重算时若所用输入与校准成员原先冻结的摘录、题名、摘要或其他用户可见输入不同，另标 human_input_drift，但按 V1 决策仍允许用户显式启用；
- 发现校准相关差异后，未分级时暂停自动精读；
- 只切换 Read 模型或其他无关配置时，运行配置快照可以变化，但校准语义基线、既有 evaluation 依据和自动精读门禁保持有效，不产生待分级变更；
- 用户标为 minor 时继承门禁；CLI 先显示仅覆盖 unresolved 范围的重算篇数与预计天数，新输入指纹使该范围制品重算；
- 多次校准相关 minor 变更后，门禁必须能从当前基线沿完整分级审计链追溯到已显式启用的通过 evaluation，不能要求完整快照 ID 与评价时相同；
...
- 真实 32 篇以及必要追加批次完成；
- 当前校准语义基线有四项指标通过的 evaluation 作为依据，或存在从该 evaluation 基线出发、仅包含已分级 minor 变更的有效继承链；重算门禁允许使用 reused_revealed_labels，但必须显著展示其非独立性质；
- 当前校准语义基线不存在待分级或 major 尚未重新通过的校准变更；
- 用户针对当前有效 evaluation 显式启用自动精读；
- 阶段 I 再次验证当前运行配置快照、校准语义基线与 evaluation、变更分级和门禁事件能够形成完整审计链。
...
2. 当前校准语义基线具有通过全部比例、反弃权与最小分母门槛的真实 evaluation 作为依据，或经已分级 minor 变更的审计链继承该依据；若复用揭晓标签，界面和记录均明确其不是独立验证；不存在待分级或未重新通过的 major 变更；
...
|只变更 Read 模型|产生新运行配置快照；校准语义基线、Screening 指纹、评价依据和门禁不变|
...
若 80% 精确率或 90% 防漏判指标不达标，可以依据已揭晓结果调整模型、提示词、Research Profile、输出契约、升级参数或建议规则，并复用原 32 篇人工标签重新计算。每次调整先由用户附理由分类为 minor 或 major，再编译并引用相应运行配置快照与校准语义基线，生成新的 Screening/复用升级制品、建议和 calibration evaluation。旧轮次不可覆盖。重算通过后，第一版允许用户据此开启自动精读，但所有状态页、报告和最终验收都必须明确标记：该结果拟合并复用了已经揭晓的标签，不是新论文上的独立验证；若重算使用的用户可见输入与原批次冻结输入不同，还必须额外显示 human_input_drift。
