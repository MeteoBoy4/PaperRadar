...
def _read_typed[T: BaseModel](
    path: Path, model: type[T], label: str
) -> tuple[bytes, T]:
    return read_yaml(path, model, label)


def _selected_materials(
    settings_path: Path,
) -> tuple[Settings, Profile | None, Material | None]:
    _, settings = _read_typed(settings_path, Settings, "settings")
    ...
    raw, profile = _read_typed(path, Profile, "profile")
...
def compile_config(settings_path: Path, database: Path) -> RuntimeConfigSnapshot:
    ...
    try:
        ...
        save_snapshot(engine, snapshot, material)
        return snapshot
    except SQLAlchemyError:
